...
co2_ppm = xco2_beta(delta_sigma_abs, beta_on/1e6, beta_off/1e6)
co2_ppm_sigma = np.var(co2_ppm)

# Arrange forward model inputs to dictionary
xco2_args = {"delta_sigma_abs": delta_sigma_abs,
             "obs_beta_att_lambda_on": obs_beta_on/1e6,
             "obs_beta_att_lambda_off": obs_beta_off/1e6}

# Covariance matrix of observations
obs_sigma = np.cov(delta_sigma_abs, beta_on/1e6, beta_off/1e6)

# Create optimal estimation object
oe = pyOE.optimalEstimation(
    ['co2_ppm'],  # state variable names
    co2_ppm,  # a priori
    co2_ppm_sigma,  # a priori uncertainty
    ['delta_sigma_abs', 'beta_att_on', 'beta_att_off'],  # measurement variable names
    co2_ppm,  # observations
    co2_ppm_sigma,  # observation uncertainty
